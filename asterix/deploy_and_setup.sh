...
# Print a direct browser link to the summary report
if [ -n "${JENKINS_URL:-}" ] && [ -n "${JOB_NAME:-}" ] && [ -n "${BUILD_NUMBER:-}" ]; then
    REPORT_URL="${JENKINS_URL%/}/job/${JOB_NAME}/${BUILD_NUMBER}/artifact/tests/integration/test_reports/test_summary_sequential.html"
    echo "=========================================="
    echo "FVTR SUMMARY REPORT:"
    echo "${REPORT_URL}"
    echo "=========================================="
fi
